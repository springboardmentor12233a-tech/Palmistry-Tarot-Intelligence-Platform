...
- When a palm reading is provided, make ARCANA SYNTHESIS
  the clearest combined interpretation of palm themes + Tarot.
- If no palm reading is provided, focus only on Tarot and
  do not invent palm themes.
...
Explain how the palm themes and the three Tarot cards reinforce, contrast, or complement one another in relation to the user's question. Keep this as the signature combined insight of the reading.
...
from app.report_service import generate_tarot_pdf
...
class TarotReportRequest(BaseModel):
    question: str
    spread: list
    interpretation: str = ""
    palm_reading: str = ""
...
@router.post("/reading")
def tarot_reading(request: TarotReadingRequest):
    """
    Draw three Tarot cards.
    Interpretation is intentionally NOT generated here.
    The frontend calls /interpret after the cards are revealed.
    """
    try:
...
@router.post("/draw")
def tarot_draw(request: TarotReadingRequest | None = None):
    """Compatibility endpoint for drawing three cards."""
    try:
        return draw_tarot_cards()
    except Exception as exc:
        raise HTTPException(
            status_code=500,
            detail=str(exc),
...
def tarot_interpret(request: TarotInterpretRequest):
    """
    Interpret the exact three cards already drawn by the frontend.
    """
    try:
        return interpret_tarot_reading(
...
@router.post("/report")
def tarot_report(request: TarotReportRequest):
    """
    Generate the polished ARCANA AI Tarot PDF using report_service.py.
    """
    try:
        if not request.question or not request.question.strip():
            raise ValueError("A question is required.")

        if not isinstance(request.spread, list) or len(request.spread) != 3:
            raise ValueError(
                "Tarot spread must contain exactly 3 cards."
            )

        output_path = generate_tarot_pdf(
            question=request.question.strip(),
            spread=request.spread,
...
@router.get("/card-image/{image_file:path}")
def tarot_card_image(image_file: str):
    """
    Serve a Tarot card image to the frontend.
    """
    image_path = TAROT_CARDS_DIR / image_file

    # Prevent path traversal outside the Tarot cards directory.
    try:
        image_path = image_path.resolve()
        cards_root = TAROT_CARDS_DIR.resolve()

        if cards_root not in image_path.parents:
            raise HTTPException(
                status_code=400,
    ...
    if not image_path.is_file():
        raise HTTPException(
            status_code=404,
            detail="Tarot card image not found.",
        )
